...
public class InfiniteProgressPanel extends JComponent implements MouseListener {
	...
	public void paintComponent(Graphics g) {
		if (started) {
			...
			for (int i = 0; i < ticker.length; i++) {
				int channel = 30 + (i + 1) * 150 / ticker.length; // 224 - 224 /
																	// (i + 1);
	...
	private Area[] buildTicker() {
		...
		for (double i = 0.0; i < (double) barsCount; i++) {
			...
			AffineTransform toCircle = AffineTransform.getRotateInstance(-i
					* 2.0d * Math.PI / ((double) barsCount), center.getX(),
					center.getY());
	...
	/**
	 * Builds a bar. It's very funny!!
	 */
	private Area buildPrimitive() {
		Rectangle2D.Double body = new Rectangle2D.Double(6 * Scale, 0 * Scale,
				30 * Scale, 12 * Scale);
		Ellipse2D.Double head = new Ellipse2D.Double(0 * Scale, 0 * Scale,
				12 * Scale, 12 * Scale);
		Ellipse2D.Double tail = new Ellipse2D.Double(30 * Scale, 0 * Scale,
				12 * Scale, 12 * Scale);
